...
TEST(AggregateByKeyTest, CountingAggregatedValues) {
    ...
    auto result =
        AsDataFlow(input)
            | AggregateByKey(
                std::size_t{0},
                // Изменён порядок параметров: первым теперь идет item, затем accumulated
                [](const std::string &item, std::size_t &accumulated) { ++accumulated; },
...
TEST(AggregateByKeyTest, AggregatingWithSeveralOutputsForEachKey) {
    ...
    auto result =
        AsDataFlow(employees)
            | AggregateByKey(
                std::vector<Employee>{},
                // Изменён порядок параметров: первым идет employee, затем accumulated
